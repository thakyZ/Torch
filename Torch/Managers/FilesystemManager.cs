...
namespace Torch.Managers
{
    public class FilesystemManager : Manager
    {
        ...
        public FilesystemManager(ITorchBase torchInstance) : base(torchInstance)
        {
            ...
            Log.Debug($"Clearing tmp directory at {TempDirectory}");
            ClearTemp();
        }
        ...
        private void ClearTemp()
        {
            foreach (var file in Directory.GetFiles(TempDirectory, "*", SearchOption.AllDirectories))
            {
                ...
                {
                    Log.Debug($"Failed to delete file {file}, it's probably in use by another process'");
                }
                catch (Exception ex)
                {
                    Log.Warn($"Unhandled exception when clearing temp files. You may ignore this. {ex}");
                }
            }
        }
